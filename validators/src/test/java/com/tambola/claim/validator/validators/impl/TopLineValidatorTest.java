...
public class TopLineValidatorTest {
    ...
    @Test
    public void testTopLineClaimAccepted() {
        List<Integer> announcedNumbers = Arrays.asList(4, 16, 48, 63, 76);
        Assertions.assertEquals(ClaimStatus.ACCEPTED, tambolaHandler.validateClaim(ClaimType.TOP_LINE, announcedNumbers));
    }


    @Test
    public void testTopLineClaimRejectedDueToLateClaim() {
        List<Integer> announcedNumbers = Arrays.asList(4, 16, 48, 63, 76, 12);
        Assertions.assertEquals(ClaimStatus.REJECTED, tambolaHandler.validateClaim(ClaimType.TOP_LINE, announcedNumbers));
    }
}
